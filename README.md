...
Скачать `MacOS-Utilities-1.4.0.dmg` из [Releases](https://github.com/sipandk-art/macos-utilities/releases),
...
Если ничего не выделено — берёт последние слова подряд: захват идёт назад
и останавливается на первом настоящем слове. Поэтому «ghbdtn rfr ltkf»
правится целиком, а в «привет как дела ghbdtn» будет тронуто только последнее
слово — на «дела» захват встаёт. Нажатое сразу второй раз возвращает как было.
...
`build/MacOS Utilities.app` и `dist/MacOS-Utilities-1.4.0.dmg`.
